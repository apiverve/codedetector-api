...
class CodedetectorValidationException implements Exception {
  final List<String> errors;

  CodedetectorValidationException(this.errors);

  @override
  String toString() => 'CodedetectorValidationException: ${errors.join("; ")}';
}
...
/// Code Detector API client.
///
/// For more information, visit: https://apiverve.com/marketplace/codedetector?utm_source=dart&utm_medium=readme
///
/// Parameters:
/// * [code] (required) - The code snippet you want to detect the programming language of
class CodedetectorClient {
  ...
  static final Map<String, ValidationRule> _validationRules = <String, ValidationRule>{
    'code': ValidationRule(type: 'string', required: true),
  };

  CodedetectorClient(this.apiKey, {
    this.baseUrl = 'https://api.apiverve.com/v1/codedetector',
    http.Client? httpClient,
  }) : _httpClient = httpClient ?? http.Client();

  /// Validates parameters against defined rules.
  /// Throws [CodedetectorValidationException] if validation fails.
  void _validateParams(Map<String, dynamic> params) {
    ...
    if (errors.isNotEmpty) {
      throw CodedetectorValidationException(errors);
    }
  }

  /// Execute a request to the Code Detector API.
  ///
  /// Parameters are validated before sending the request.
  Future<CodedetectorResponse> execute(Map<String, dynamic> params) async {
    // Validate parameters
    _validateParams(params);
    if (apiKey.isEmpty) {
      throw CodedetectorException('API key is required. Get your API key at: https://apiverve.com');
    }
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        return CodedetectorResponse.fromJson(json);
      } else if (response.statusCode == 401) {
        throw CodedetectorException('Invalid API key');
      } else if (response.statusCode == 404) {
        throw CodedetectorException('Resource not found');
      } else {
        throw CodedetectorException('API error: ${response.statusCode}');
      }
    } catch (e) {
      if (e is CodedetectorException) rethrow;
      throw CodedetectorException('Request failed: $e');
    }
  }
...
/// Exception thrown by the Code Detector API client.
class CodedetectorException implements Exception {
  final String message;

  CodedetectorException(this.message);

  @override
  String toString() => 'CodedetectorException: $message';
}
